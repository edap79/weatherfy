...
{
    var rainData = [String: [String: Double]]()
    ...
    func loadData()
    {
        let townsData = data.components(separatedBy: "\n")
        
        for townData in townsData
        {
            let townDataComponents = townData.components(separatedBy: ":")
            let townName = townDataComponents[0].lowercased()
            let monthValues = townDataComponents[1].lowercased()
            let monthComponents = monthValues.components(separatedBy: ",")
            var values = [String: Double]()
            
            for monthData in monthComponents
            {
                let monthComponent = monthData.components(separatedBy: " ")
                let monthName = monthComponent[0]
                let monthValue = Double(monthComponent[1])
                
                values[monthName] = monthValue
            }
            
            self.rainData[townName] = values
        }
    }
    
    func getRainData(town: String) -> [Double]
    {
        if let townData = self.rainData[town.lowercased()]
        {
            let values = townData.map({$0.value})
            var summed = values.reduce(0, +)
            let mean = Double(summed) / Double(values.count)
            
            let squaredDeviations = values.map{ pow(Double($0) - mean, 2)}
            summed = squaredDeviations.reduce(0, +)
            let variance = Double(summed) / Double(squaredDeviations.count)
